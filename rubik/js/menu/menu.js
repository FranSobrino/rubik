var perspectiveMenu = (function() {
	...
	function init() {
		var showMenu = $('#showMenu'),
			...
			container = perspectiveWrapper.find('.container'),
		...
		container.on('click', function( ev ) {
			if( perspectiveWrapper.hasClass('animate') ) {
				var onEndTransFn = function( ev ) {
					if( support && ( ev.target.className !== 'container' || ev.originalEvent.propertyName.indexOf( 'transform' ) == -1 ) ) return;
		...
		perspectiveWrapper.on('click', function( ev ) { return false; } );
	}
